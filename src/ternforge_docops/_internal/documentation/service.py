...
_VERIFICATION_PAGE_DIR = Path("verification/_generated")
_LIVING_SOURCE_HEADER = ".. Generated by Ternforge DocOps; do not edit.\n\n"
_VERIFICATION_SOURCE_HEADER = ".. Generated by Ternforge DocOps; do not edit.\n\n"
...
def _reset_verification_page_tree(root: Path) -> None:
    """Remove an interrupted verification page tree only when DocOps owns it."""
    if not root.exists():
        return
    files = [path for path in root.rglob("*") if path.is_file()]
    owns_tree = all(
        path.suffix == ".rst"
        and path.read_text(encoding="utf-8").startswith(_VERIFICATION_SOURCE_HEADER)
        for path in files
    )
    if not owns_tree:
        message = f"DocOps reserved verification page tree exists: {root}"
        raise RuntimeError(message)
    shutil.rmtree(root)


def _verification_page_target(docs_root: Path, docname: str) -> Path:
    """Resolve one verification page inside the reserved generated tree."""
    relative = Path(f"{docname}.rst")
    try:
        relative.relative_to(_VERIFICATION_PAGE_DIR)
    except ValueError as error:
        message = f"Verification page is outside reserved tree: {docname}"
        raise RuntimeError(message) from error
    return docs_root / relative


def _materialize_verification_pages(
    repo_root: Path,
    docs_root: Path,
    evidence: VerificationEvidencePaths,
    generated_dirs: list[Path],
) -> None:
    """Materialize narrative pages for unit/property/integration/e2e checks."""
    root = docs_root / _VERIFICATION_PAGE_DIR
    _reset_verification_page_tree(root)
    if evidence.junit is None:
        return
    resolved = evidence.junit.resolve()
    ...
    if not pages:
        return
    for page in pages:
        target = _verification_page_target(docs_root, page.docname)
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(
            _VERIFICATION_SOURCE_HEADER + page.source,
            encoding="utf-8",
        )
    generated_dirs.append(root)
...
def _materialized_sources(
    ...
    _materialize_verification_pages(
        repo_root,
        docs_root,
        evidence,
        generated_dirs,
    ...
    try:
        ...
        with suppress(OSError):
            (docs_root / _VERIFICATION_PAGE_DIR.parent).rmdir()
        with suppress(OSError):
            trace_dir.rmdir()
...
def _prepare_living_specifications(
    ...
    if evidence.allure_results is None:
        message = "Living Specifications require retained Allure results"
        raise ValueError(message)
    curated = temp / "curated"
    curate_results(evidence.allure_results, curated_results=curated)
...
def build_html(
    ...
    *,
    ...
    evidence: VerificationEvidencePaths | None = None,
    live_examples: bool = False,
) -> Path:
    ...
    retained = evidence or VerificationEvidencePaths()
    with _materialized_sources(root, docs_root, retained):
        _run_sphinx(
            root,
            docs_root,
            output_root,
...
def build_dossier(
    ...
    *,
    ...
    retained = evidence or VerificationEvidencePaths()
    if retained.allure_results is None:
        with _materialized_sources(root, docs_root, retained):
...
def build_portal(
    ...
    *,
    ...
    if evidence.allure_results is None:
        message = "Portal build requires retained Allure results"
        raise ValueError(message)
    docs_root = root / "docs"
    output_root = output or docs_root / "_build" / "html"
    with tempfile.TemporaryDirectory(prefix="ternforge-docops-allure-") as temp_dir:
